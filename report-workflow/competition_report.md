...
系统已完成全部硬件接线（SHT31、PN532、W25Q128、ATGM336H GPS、Air780E 4G）和软件驱动开发。5个外设模块均通过条件编译宏切换至真实HAL驱动模式，系统已成功编译并烧录至NUCLEO-H7A3ZI-Q开发板运行。云端EMQX Cloud部署完成，自建Web控制台可通过GitHub Pages访问。

### 3.2 工程成果

#### 3.2.2 电路成果

> 实物照片待拍摄：整机接线正面照片、传感器模块特写

系统采用NUCLEO-H7A3ZI-Q开发板配合外接模块：

- 面包板/杜邦线连接各传感器模块
- SHT31和PN532共享I2C1总线（PB8/PB9）
- W25Q128通过SPI1连接（PA4-PA7）
- ATGM336H通过USART2连接（PD5/PD6）
- Air780E通过USART1连接（PA9/PA10），独立4.0V供电

#### 3.2.3 软件成果

- **编译结果**：0 Error, 0 Warning（Keil MDK + ARM Compiler 6.21）
- **FreeRTOS运行**：6任务正常调度，3队列通信无阻塞
- **云端控制台**：dashboard.html通过GitHub Pages部署，实时温湿度/GPS/电池数据显示，三指令按钮下发

> 硬件验证进行中，测试数据待补充。

### 3.3 特性成果

> 以下指标基于设计参数和代码实现，实测数据待系统集成测试后补充

| 功能 | 设计指标 | 实现状态 |
|------|---------|---------|
| NFC激活 | 标签靠近<1s响应 | 代码已实现 |
| GPS定位 | 冷启动<3min | 代码已实现 |
| 温湿度采样 | 精度±0.3°C/±2%RH | 代码已实现 |
| 4G上报延迟 | <500ms | 代码已实现 |
| 离线缓存 | 128条记录 | 代码已实现 |
| 低功耗 | Stop模式<100μA | 待实现 |
