...
```
        ...
        ▼                                                  │
  ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌─────────┐│
  │  SLEEP   │──▶│NFC_ACTIVE│──▶│GPS_LOCATE│──▶│ENV_SAMPLE││
  │ 低功耗   │   │ PWRKEY   │   │ GPS请求  │   │ SHT31请求││
  │ 绿灯慢闪 │   │ Air780E开│   │ 等待7s   │   │ 等待5s   ││
  └──────────┘   └──────────┘   └──────────┘   └────┬────┘│
       ▲                                            │      │
       │                          ┌──────────┐       │      │
       │                          │  ALARM   │◀──超阈值     │
       ...
       │                                       │    │      │
       │  ┌──────────┐   ┌──────────┐   ┌─────┴────┴──┐   │
       └──│RETURN_   │◀──│WAIT_CLOUD│◀──│   UPLOAD    │───┘
          │SLEEP     │   │ 等待指令 │   │ 红灯 MQTT   │
          │ Stop Mode│   │ 30s 窗口 │   │ 失败→Flash │
          └──────────┘   └──────────┘   └─────────────┘
```

| 状态 | LED | 动作 | 超时/触发 |
|------|:---:|------|------|
| SLEEP | 🟢慢闪 | 队列等待激活；4G关断 | NFC/按键事件 |
| NFC_ACTIVE | 🟡 | PWRKEY开机；4G初始化 | — |
| GPS_LOCATE | — | 置位请求→等待GPS数据入队 | 7s→下一状态 |
| ENV_SAMPLE | — | 置位请求→SHT31采样→超阈值→ALARM | 5s→下一状态 |
| ALARM | 🟡快闪 | 蜂鸣器断续鸣叫 | 手动/指令清除 |
| UPLOAD | 🔴亮 | MQTT PUBLISH telemetry；失败写Flash | 完成后→WAIT_CLOUD |
| WAIT_CLOUD | — | `queue_cloud_cmd` 等 HOLD/RETURN/CONTINUE | 30s→RETURN_SLEEP |
| RETURN_SLEEP | — | Air780E 断电；进入 Stop Mode | 等待下次 NFC |
...
| 驱动 | 接口 | 协议/命令 | 校验 | 关键参数 |
|------|------|---------|------|------|
| SHT31 | I2C(0x44) | 0x2400 测量 → 读6B | CRC8 | `T=-45+175·raw/65535` |
| PN532 | I2C(0x24) | 前导码+LEN+LCS+TFI+DATA+DCS | LCS+DCS | SAMConfiguration→InListPassiveTarget |
| W25Q128 | SPI(10MHz) | Page Program(256B) / Sector Erase(4KB) | Magic `0xA5C35A3C` | JEDEC 0xEF17, 128槽×32B |
| GPS | USART2(9600) | $GNRMC / $GNGGA NMEA | 字段计数+状态位 | ddmm.mmmm→十进制 |
| Air780E | USART1(115200) | AT 指令集 | 响应码匹配 | PWRKEY>1s 低脉冲, CSQ≥20 |
| 按键 | GPIO EXTI | PC13 Falling | 200ms tick 去抖 | ISR 仅入队, 无阻塞 |
...
| # | 测试项 | 理论预期 | 实测结果 | 闭环 |
|:---:|------|------|------|:---:|
| 1 | SHT31 温度 | `T=-45+175·raw/65535` | 24-25°C（室内合理） | ✅ 传感器手册→代码→串口 |
| 2 | PN532 UID | ISO14443A 防冲突 | init OK / 标签待测 | ⚠️ 半闭环 |
| 3 | W25Q128 JEDEC | 0xEF17 (Winbond) | 0xEF 40 18 | ✅ 手册→SPI→实测 |
| 4 | GPS 定位 | CEP50 <2.5m | 待户外测试 | ⚠️ NMEA→坐标→待户外实测 |
| 5 | Air780E 信号 | CSQ≥20 优秀 | CSQ=27~28 | ✅ AT指令→天线→实测 |
| 6 | MQTT 延迟 | <500ms | 72 bytes 到达 dashboard | ✅ TCP→PUBLISH→Web端确认 |
| 7 | 云指令 | HOLD/RETURN/CONTINUE | 代码就绪，待联调 | ⚠️ |
| 8 | 断网缓存 | Magic→写入→擦除→读取 | ❌ W25Q128 补传路径缺失 | ❌ |
| 9 | Stop Mode | <100μA (手册 3.3.3节) | ❌ stub→待真实现 | ❌ |
| 10 | 24h 稳定 | 运行中 FreeRTOS 无泄漏 | ❌ 未测 | ❌ |

> 闭环✅：设计理论 → 代码实现 → 实测数据三者吻合。这是特等奖标配。

### 8.2 实物照片清单

| 编号 | 照片内容 | 用途 |
|:---:|------|------|
| 📷1 | 整机接线正面（所有模块+杜邦线） | 报告 3.2.2 电路成果 |
| 📷2 | 整机接线 45° 斜拍 | 报告 3.1 整体介绍 |
| 📷3 | SHT31 特写 + 串口温度读数 | 温湿度验证 |
| 📷4 | W25Q128 特写 + JEDEC 串口截图 | Flash 验证 |
| 📷5 | Air780E 接线 + AT 指令串口截图 | 4G 通信验证 |
| 📷6 | dashboard 实时数据截图（手机端） | 云端交互验证 |
| 📷7 | 万用表测电流照片 | 低功耗实测 |

> ⚠️ 以上照片需拍摄后嵌入 HTML 或插入 PDF 报告。
...
### 8.3 MQTT 通信时序图（GPT 可画）

> 复制以下结构给 GPT，让它画 MQTT 端到端通信时序图：

```
参与者（从左到右）: STM32/Air780E, EMQX Broker, dashboard.html

时序:
1. STM32→Broker: AT+CIPSTART TCP 1883
...
loop 每10s循环:
  7. STM32→Broker: PUBLISH cargo/telemetry {temp,hum,lat,lon,csq}
  8. Broker→dashboard: 推送 telemetry (WebSocket)
  9. dashboard: 更新仪表盘+地图

opt 用户操作:
  10. dashboard→Broker: PUBLISH cargo/cmd {"cmd":"HOLD"}
  11. Broker→STM32: 推送 cmd
  12. STM32→STM32: MQTT_ParsePublishBuffer→queue_cloud_cmd→状态机执行
```
...
| 图号 | 内容 | 位置 | 类型 |
|:---:|------|:---:|------|
| 1 | 硬件系统框图（6外设环绕MCU） | §8.1 | 结构图 |
| 2 | 软件分层架构（App→FreeRTOS→Driver→HAL） | §8.2 | 层次图 |
| 3 | 状态机流转图（8状态+事件触发） | §3.4 | 流程图 |
| 4 | MQTT 端到端时序图 | §8.3 | 时序图 |
| 5 | 前后端交互架构（设备→云端→前端） | §5.3 | 架构图 |
| 6 | 电源树（USB→充电→电池→两路降压） | §2.3 | 拓扑图 |

> 以上 6 张图 + 7 张实物照片（§8.2）= 报告全部图表。可直接复制各节的结构给 GPT/ChatGPT 绘制。
