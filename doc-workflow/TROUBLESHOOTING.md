...
```bash
# 用 sed 或手动在 Keil 里改 Include Path + port.c 路径
```

**Step 2: 恢复 FPU（否则浮点运算走软件模拟）**
...
**Step 3: 恢复 Air780E PWRKEY（否则 4G 模块不工作）**

`Core/Inc/main.h`：
```c
#define AIR780E_PWRKEY_Pin GPIO_PIN_0      // 不是 LD1_GREEN！
#define AIR780E_PWRKEY_GPIO_Port GPIOB
```

`Core/Src/gpio.c`：所有 `LD1_GREEN_Pin` → `AIR780E_PWRKEY_Pin`

> 如果队友 pull 后不需要 Generate Code，这三步**不用做**——只有运行了 CubeMX → Generate Code 才需要。
